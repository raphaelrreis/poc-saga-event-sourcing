...
public class KafkaObjects {
	...
	private String topicoDePedidos;
	...
	@Bean
	KafkaTemplate<String, VendaEvent> vendaKafkaTemplate() {
	...
	@Bean
	DefaultKafkaProducerFactory<String, VendaEvent> producerFactory() {
	...
	@Bean
	KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<String, VendaValidadaEvent>> vendaValidadaKafkaListener() {
		ConcurrentKafkaListenerContainerFactory<String, VendaValidadaEvent> factory =
				new ConcurrentKafkaListenerContainerFactory<>();
		factory.setConsumerFactory(vendaValidadaConsumerFactory());
		return factory;
	}
	...
	// para poder fazer o distributed tracing
	@Bean
	ConsumerFactory<String, PedidoSolicitadoEvent> pedidoConsumerFactory() {
	...
	// para poder fazer o distributed tracing
	@Bean
	ConsumerFactory<String, VendaValidadaEvent> vendaValidadaConsumerFactory(){
		return new DefaultKafkaConsumerFactory<>(
				consumerConfigs(VendaValidadaEvent.class),
				new StringDeserializer(),
				jsonDeserializer(VendaValidadaEvent.class));
	}

	public <T> JsonDeserializer<T> jsonDeserializer(Class<T> clazz){
